...
function add_delete_button($formset_form) {
    $formset_form.find('input:checkbox[id$=DELETE]')
     .each(function () {
        var $checkbox = $(this);
        var $deleteLink = $(
            '<button class="delete btn btn-sm btn-default mb-3  ml-2"><i class="fa fa-trash ">X</i></button>'
        );
...
$(document).on("click", ".add-demand_file-formset", function() {
    add_formset_fields($(this))
})
